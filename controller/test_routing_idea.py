...
def main():
    ...
    while(True):
        ts = load.timescale()
        t = ts.now()
        # print('UTC date and time:', t.utc_strftime('%Y-%m-%d %H:%M:%S'))
        # print('UTC date and time:', t.utc_datetime())
        old_gsls = G["GSL_Connectivity"][:]
        G = graph_add_GSLs_a_single_node(G["Graph"], available_satellites_by_name, actual_sat_number_to_counter, ground_stations[27], t, 1, "BASED_ON_DISTANCE_ONLY_GRAPH")
        print('UTC date and time:', t.utc_strftime('%Y-%m-%d %H:%M:%S'), "best sat", G["sid"], "distance: ", G["distance"])
